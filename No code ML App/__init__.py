...
path = [os.getcwd()+'\\Images\\classi', os.getcwd()+'\\Images\\reg', os.getcwd()+'\\Images\\cluster']
for i in path:
...
def run_classification_model(df, data_split, seed, label, id, column):
    ...
    if proceed and data_type is "No":
        ...
        pred=class_model.prediction(best)
        st.write(pred)
        st.download_button(label="Download data as CSV",data=convert_df_to_csv(pred),file_name='predicted(class)_test_data.csv',mime='text/csv')
        ...
        #eval1=st.checkbox("Evaluation metrics")
        #if eval1:
            #plot_graph(best, '')
...
def run_regression_model(df, data_split, seed, label, id, column):
    ...
    if proceed and data_type is "No":
        ...
        st.header("Prediction")
        pred=reg_model.prediction(best)
        st.write(pred)
        st.download_button(label="Download data as CSV",data=convert_df_to_csv(pred),file_name='predicted(reg)_test_data.csv',mime='text/csv')

        '''st.header("Plot")
        st.set_option('deprecation.showPyplotGlobalUse', False)
        st.pyplot(classify.plot_model(best, 'feature'))'''
        ...
        with st.container():
            ...
            st.code(best)
            st.subheader('Performance of '+ results.Model[0])
            '''st.pyplot(plot_graph(best, 'vc'))
            st.pyplot(plot_graph(best, 'error'))
            st.pyplot(plot_graph(best, 'residuals'))'''
...
def run_cluster_model(df, data_split, seed,  id, column):
    ...
    if proceed and data_type is "No":
        ...
        st.header("Prediction")
        st.write(result)
        st.download_button(label="Download data as CSV",data=convert_df_to_csv(result),file_name='predicted(clust)_test_data.csv',mime='text/csv')
...
# Main page
st.title("Nocode ML")
st.write("Developed by: Guhanesvar M")
